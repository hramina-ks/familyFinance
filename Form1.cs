...
namespace familyFinance
{
    public partial class frmFinance : Form
    {
        ...
        {
            string sSaveFile = ""; //тут будет путь к файлу
        ...
        {
            string sLoadFile = ""; //тут будет путь к файлу
        ...
        {
            if (MessageBox.Show("Если вы не сохранили данные в файл, они пропадут. Очистить?", "Очистка", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                Income.Clear();
                Expense.Clear();
                rtbOutput.Text = TextBoxLine(Income.Output(), Convert.ToString(Income.SumOfElements()), Expense.Output(), Convert.ToString(Expense.SumOfElements()), Convert.ToString(Income.SumOfElements() - Expense.SumOfElements()));
